...
inline const char *toString(TraceLevel level)
{
    static const char *str[] = {"E", "W", "I", "D", "T"};
    static_assert(
        sizeof(str) / sizeof(str[0])
        == int(TraceLevel::end) - int(TraceLevel::begin));
    return TraceLevel::end > level && TraceLevel::begin <= level
        ? str[int(level)]
        : "?";
}
...
inline void traceImpl(std::ostream &) { }
...
inline TraceLevel currTraceLevel()
{
...
inline void trace(TraceLevel traceLevel, const std::ostringstream &oss)
{
    ...
    std::ostream &dst
        ...
        ? std::cerr
        : std::cout;
    dst << '[' << ::getpid() << '|' << ::gettid() << '|' << toString(traceLevel)
        << "] " << oss.str() << std::endl;
}
...
void trace(TraceLevel traceLevel, const T_n &...tail)
{
    ...
    #define TRACE(traceLevel, ...)                                             \
        ...
        {                                                                      \
            trace(                                                             \
                traceLevel, __FILE__, ':', __LINE__, ' ', __FUNCTION__, ' ',   \
                __VA_ARGS__);                                                  \
        } while (false)
