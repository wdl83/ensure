...
inline std::string toString(DbgInfo dbgInfo)
{
    std::string str;
...
struct ExceptionImpl : public EnsureException, public BaseException
{
    ...
    ExceptionImpl(DbgInfo info, const char *msg)
        ...
        , BaseException{
              '#' + std::to_string(id) + ' ' + EnsureException::toString() + ' '
              + msg}
    { }
};
...
#define ENSURE_IMPL(cond, ExceptionType, ...)                                  \
    ...
    {                                                                          \
        ...
        {                                                                      \
            TRACE(                                                             \
                TraceLevel::Error, "throw ", TO_STRING(cond), ", ",            \
                TO_STRING(ExceptionType), ", ", __VA_ARGS__);                  \
            throw (ExceptionType){                                             \
...
#define vENSURE(cond, ExceptionType, ...)                                      \
    ENSURE_IMPL(cond, ExceptionType, __VA_ARGS__)
